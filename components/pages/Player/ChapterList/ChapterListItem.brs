...
sub showContent()
    ...
    if m.title <> invalid then m.title.text = FirstNonEmpty([item.title], "")
    if m.duration <> invalid then m.duration.text = FirstNonEmpty([item.description], "")
